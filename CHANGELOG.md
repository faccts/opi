...
- Added `to_ase()` to convert a `Structure` into an ASE `Atoms` object (#XXX)
- `Structure.from_ase()` now falls back to the `charge` and `multiplicity` entries of `Atoms.info` if ASE's per-atom `initial_charges` / `initial_magnetic_moments` arrays are unset (#XXX)
### Changed
